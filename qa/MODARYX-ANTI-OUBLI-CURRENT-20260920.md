...
| Compte Fondateur / Administrateur MODARYX | **EN COURS — compte Fondateur Auth0 DEV attribué ; preuves runtime privilégiées restantes** | PR #142 a intégré l'autorité serveur/UI. Work a confirmé sur Auth0 `modaryx-dev` EU l'attribution `modaryx:founder` via le rôle `MODARYX Founder` au compte Google DEV existant « alou », puis une session renouvelée avec `authority.role=founder` et les capacités founder/administration/modération/recours. Aucun secret/token exposé. Les micro-preuves runtime modération/recours restent ouvertes à cause de `TypeError: fetch is not a function` dans le contexte authentifié ; cookie HttpOnly non extrait. Sources : `qa/MODARYX-FOUNDER-ADMIN-ACCESS-20260921.md` + `qa/MODARYX-FOUNDER-AUTH0-DEV-EVIDENCE-20260921.md`. |
