...
| Profils publics éditables | **EN COURS — endpoint distant codé, service non provisionné** | Contrat profil acquis ; endpoint authentifié + lecture publique `public` uniquement ajoutés et protégés par run `35520142277`, marker `PASS_TARGETED_REMOTE_WRITE_ENDPOINTS`. D1/Auth0/Turnstile réels restent à provisionner. |
...
| Backend communautaire | **EN COURS — fondation + endpoints distants codés, ressources non provisionnées** | Fondation DEV prouvée + endpoints profil/communauté protégés par `PASS_TARGETED_REMOTE_WRITE_ENDPOINTS`; aucune ressource Cloudflare réelle, aucun binding/secret, aucune écriture réseau prouvée. |
