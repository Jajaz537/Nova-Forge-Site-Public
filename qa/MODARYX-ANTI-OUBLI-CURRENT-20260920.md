...
| Signatures / attestations de provenance | **EN COURS — vérification + trust-anchor gate acquis ; signer réel non connecté** | Les contrats et le moteur cryptographique restent protégés par `PASS_TARGETED_PROVENANCE_RECEIPT_CONTRACTS`, `PASS_TARGETED_SIGNATURE_ATTESTATION_CONTRACT` et `PASS_TARGETED_SIGNATURE_VERIFICATION_ENGINE`. Le batch Super Nova ajoute `data/trusted-signers.json` vide/fail-closed et `PASS_TARGETED_TRUSTED_SIGNER_GATE` : une signature mathématiquement valide n’est fiable que si sa clé publique est explicitement approuvée, valide et non révoquée. Aucun trust anchor ni signer de production n'est publié. |
