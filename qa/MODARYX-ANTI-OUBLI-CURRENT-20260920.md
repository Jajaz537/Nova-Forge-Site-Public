...
**Base Git vérifiée avant la canonisation multi-rôle :** `design/modaryx-premium-hd-20260914-work` @ `a63684d7ffc6d4cebd86926b8bf252e258624f84`.  
...
|---|---|---|
| Comptes / authentification / passkeys réels | **EN COURS — préparation source passkey finale acquise ; cérémonie provider/appareil réelle non prouvée** | Provider DEV login/callback/session déjà réel ; `qa/MODARYX-PASSKEY-PROVIDER-READINESS-20260921.md` + marker `PASS_TARGETED_PASSKEY_PROVIDER_READINESS` verrouillent Universal Login, absence de WebAuthn maison, aucune clé privée, récupération/révocation et handoff minimal. Restent activation/configuration passkey fournisseur et vraie cérémonie sur appareil approprié. |
...
| Signatures / attestations de provenance | **EN COURS — contrat + moteur de vérification acquis ; signer réel non connecté** | Contrats provenance protégés par `PASS_TARGETED_PROVENANCE_RECEIPT_CONTRACTS` ; attestation documentée dans `qa/MODARYX-SIGNATURE-ATTESTATION-CONTRACT-20260921.md` avec marker `PASS_TARGETED_SIGNATURE_ATTESTATION_CONTRACT`. PR #128 ajoute `functions/_lib/attestation.mjs` + `qa/MODARYX-SIGNATURE-VERIFICATION-ENGINE-20260921.md`, marker `PASS_TARGETED_SIGNATURE_VERIFICATION_ENGINE` : fingerprint de clé publique, payload canonique, ES256/EdDSA, révocation et tampering fail-closed. Aucun signer réel ni clé de confiance opérationnelle n'est connecté. |
...
| Storage Resolver | **EN COURS — contrat + moteur décisionnel acquis ; service/transport non connecté** | Contrat provider-neutral protégé par `PASS_TARGETED_STORAGE_REPAIR_CONTRACTS`. PR #128 ajoute `functions/_lib/storage-repair-engine.mjs` + `qa/MODARYX-STORAGE-REPAIR-DECISION-ENGINE-20260921.md`, marker `PASS_TARGETED_STORAGE_REPAIR_DECISION_ENGINE` : manifeste + digest exacts, alias mutable rejeté, choix d’origine fail-closed. Aucun transport/résolveur distant réel n’est connecté. |
| Repair Network | **EN COURS — protocole + moteur décisionnel acquis ; exécution distante non connectée** | PR #128 partage le moteur `storage-repair-engine.mjs`, marker `PASS_TARGETED_STORAGE_REPAIR_DECISION_ENGINE` : réparation seulement sur copie vérifiée au digest exact ; releases withdrawn/revoked restent non distribuables ; aucune substitution silencieuse. Le transport/réseau de réparation réel reste absent. |
...
| Guide MODARYX connecté | **EN COURS — contrat + gate de consentement acquis ; service réel non connecté** | Contrat `schemas/modaryx-guide-connection.schema.json` + `PASS_TARGETED_GUIDE_OS_BRIDGE_CONTRACTS`. PR #128 ajoute `functions/_lib/integration-consent.mjs` + `qa/MODARYX-INTEGRATION-CONSENT-ENGINE-20260921.md`, marker `PASS_TARGETED_INTEGRATION_CONSENT_ENGINE` : consentement explicite, grants ⊆ scopes demandés, endpoint HTTPS, aucune session/credential partagés. Service Guide réel toujours absent. |
