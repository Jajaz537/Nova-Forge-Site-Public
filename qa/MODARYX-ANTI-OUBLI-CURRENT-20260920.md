...
| Compte Fondateur / Administrateur MODARYX | **EN COURS — autorité serveur et UI intégrées ; attribution Auth0 réelle restante** | `modaryx:founder` / `modaryx:admin`, héritage serveur administration + modération + recours, permissions inconnues refusées par défaut, secrets infrastructure exclus. Source : `qa/MODARYX-FOUNDER-ADMIN-ACCESS-20260921.md`. Fermeture réelle : attribution du rôle au compte Fondateur DEV + nouvelle session + micro-preuves Preview. |
