...
| Signatures / attestations de provenance | **EN COURS — contrat d’attestation/signature acquis ; signer réel non connecté** | Contrats provenance historiques toujours protégés par `PASS_TARGETED_PROVENANCE_RECEIPT_CONTRACTS` ; `schemas/signature-attestation.schema.json` + `qa/MODARYX-SIGNATURE-ATTESTATION-CONTRACT-20260921.md`, marker `PASS_TARGETED_SIGNATURE_ATTESTATION_CONTRACT` : signature détachée liée au SHA-256, algorithmes bornés, état verified/failed/revoked et clé privée interdite. Aucun signer réel ni clé de confiance n'est encore connecté. |
...
| Guide MODARYX connecté | **EN COURS — contrat de connexion acquis ; service réel non connecté** | `schemas/modaryx-guide-connection.schema.json` + `qa/MODARYX-GUIDE-OS-BRIDGE-CONTRACT-20260921.md`, marker `PASS_TARGETED_GUIDE_OS_BRIDGE_CONTRACTS` : consentement explicite, moindre privilège, aucune session/credential implicitement partagés. Le service Guide réel reste absent. |
| Pont Nova Forge OS | **EN COURS — contrat de pont acquis ; runtime OS non connecté** | `schemas/nova-forge-os-bridge.schema.json` + marker `PASS_TARGETED_GUIDE_OS_BRIDGE_CONTRACTS` : frontière `modaryx-web → nova-forge-os`, consentement et permissions explicites, aucun account linking/session/credential implicite. Le runtime côté Nova Forge OS reste une dépendance distincte. |
