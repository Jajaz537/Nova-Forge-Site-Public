...
| Signatures / attestations de provenance | **EN COURS — contrat + moteur de vérification acquis ; signer réel non connecté** | Contrats provenance protégés par `PASS_TARGETED_PROVENANCE_RECEIPT_CONTRACTS` et attestation par `PASS_TARGETED_SIGNATURE_ATTESTATION_CONTRACT`. PR #128 ajoute `functions/_lib/attestation.mjs` + `qa/MODARYX-SIGNATURE-VERIFICATION-ENGINE-20260921.md`, marker `PASS_TARGETED_SIGNATURE_VERIFICATION_ENGINE` : fingerprint de clé publique, payload canonique, ES256/EdDSA, révocation et tampering fail-closed. Aucun signer réel ni clé de confiance opérationnelle n'est connecté. |
