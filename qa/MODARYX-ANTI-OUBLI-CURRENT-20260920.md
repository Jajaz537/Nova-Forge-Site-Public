...
| Publication / modération distante | **EN COURS — contrats receipts acquis, backend distant non connecté** | Contrats notice/décision/appel/export prouvés par `PASS_TARGETED_PROVENANCE_RECEIPT_CONTRACTS`; backend sécurisé, rôles, politique d'abus et exécution distante restent à connecter. |
| Signatures / attestations de provenance | **EN COURS — contrats provenance acquis, signer/attestation réel non connecté** | Publication receipt fail-closed prouvé par `PASS_TARGETED_PROVENANCE_RECEIPT_CONTRACTS`; un `receipt:` bien formé n'atteste toujours aucune authenticité. |
