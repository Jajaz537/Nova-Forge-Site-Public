...
**Base Git vérifiée avant la canonisation modération :** `design/modaryx-premium-hd-20260914-work` @ `5403b265f3fd3300960db476ffbbe9bce1f0649c`.  
...
| Publication / modération distante | **EN COURS — moteur complet codé/prouvé ; activation fournisseur DEV restante** | Ingestion DEV réelle déjà prouvée. PR #121–#123 ajoutent file modérateur, décisions, publication/retrait, surface publique, suivi auteur et recours/issue avec permissions séparées. Restent migration D1 0003 + RBAC Auth0 et micro-preuve fournisseur réelle avant fermeture. |
| UI communauté distante modérée | **TERMINÉ — envoi + surface publique + suivi/recours ciblés** | Base UI PR #112 ; PR #122 ajoute la surface publique fail-closed ; PR #123 ajoute suivi auteur et recours. Candidat #123 : source `35544873531` success, Static Premium HD/Firefox/WebKit/reflow/a11y et Catalog Community States success. La preuve provider réelle du cycle modération reste séparée. |
...
| Backend communautaire | **EN COURS — Provider DEV + writes réels ; moteur modération/recours codé, activation réelle restante** | D1/Auth0/Turnstile et writes réels déjà prouvés. Le moteur modération/publication/recours est maintenant livré en code avec migration 0003 et RBAC prévu ; restent application 0003, permissions Auth0 et micro-preuve bout-en-bout réelle. |
...
| Provider DEV réel + micro-proofs bout-en-bout | **TERMINÉ — périmètre profils + ingestion communautaire** | Login/callback/session, `profile-write`, `community-write`, fail-closed Turnstile/origine et profil public DEV sont prouvés. Cette fermeture ne couvre pas encore le nouveau lot migration 0003 + RBAC + cycle modération/recours. |
| Moteur modération / publication / recours | **TERMINÉ — code + preuves ciblées ; provider réel restant** | PR #121–#123 : migration 0003, permissions séparées, décisions, public feed, suivi auteur, recours et issues ; marker `PASS_TARGETED_MODERATION_PUBLICATION_ENGINE` vert sur run `35544873531`. La preuve réelle D1/RBAC/décision/recours reste **EN COURS** dans la ligne Publication / modération distante. |
...
3. Fermer les écarts produit encore réels sans les simuler : activer/prover le moteur modération/publication/recours en DEV, passkey finale, corpus réels, distribution, Storage Resolver, Repair Network, Guide et pont OS selon leurs dépendances.
