...
| Compte Fondateur / Administrateur MODARYX | **EN COURS — compte Fondateur Auth0 DEV attribué ; modération runtime TERMINÉE ; recours runtime restant** | PR #142 a intégré l'autorité serveur/UI. Work a confirmé sur Auth0 `modaryx-dev` EU l'attribution `modaryx:founder` via le rôle `MODARYX Founder` au compte Google DEV existant « alou », puis une session renouvelée avec `authority.role=founder` et les capacités founder/administration/modération/recours. Aucun secret/token exposé. La preuve runtime modération Fondateur est désormais acquise après réauthentification : HTTP 200, `held-for-review`, `received`, receipt créé et cleanup confirmé. La preuve runtime recours reste ouverte ; cookie HttpOnly non extrait. Sources : `qa/MODARYX-FOUNDER-ADMIN-ACCESS-20260921.md` + `qa/MODARYX-FOUNDER-AUTH0-DEV-EVIDENCE-20260921.md`. |
| Micro-surface Fondateur DEV temporaire | **EN COURS — preuve runtime uniquement ; retrait obligatoire après preuve** | `/founder-proof-dev` : Preview stable exact uniquement, session `modaryx:founder`, formulaires HTML sans JS, vrais handlers modération/recours, fixtures D1 éphémères et nettoyage requis. Source : `qa/MODARYX-FOUNDER-DEV-PROOF-SURFACE-20260921.md`. **Ne doit jamais rester dans la VF ni être promue en production.** |
...
| Préparation fournisseur DEV | **TERMINÉ — preuve ciblée provider-independent** | `qa/MODARYX-DEV-PROVIDER-READINESS-20260920.md` ; ordre D1 0001→0002→0003, hostname Turnstile, Auth0 RS256/audience et RBAC `community:moderate` + `community:appeals-review` + `modaryx:admin` + `modaryx:founder` verrouillés par le checker ciblé. |
...
| CWV représentatifs | **PREUVE MANQUANTE** | Work Phase 2 : rapport PageSpeed frais acquis mais « Aucune donnée » utilisateur réelle/CrUX ; les excellents résultats labo ne ferment pas le terrain. Reprise du 21/09 : tentative API `HTTP 429 RESOURCE_EXHAUSTED / RATE_LIMIT_EXCEEDED`, interface sans donnée terrain exploitable ; aucune substitution par Lighthouse labo. |
