...
class Element {constructor(){this.children=[];this.dataset={};this.hidden=false;this.disabled=false;this.events={};this.textContent='';} replaceChildren(...x){this.children=x;} append(...x){this.children.push(...x);} addEventListener(k,f){this.events[k]=f;} focus(){this.focused=true;}}
