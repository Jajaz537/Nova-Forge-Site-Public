...
const OUT=process.env.MODARYX_FIREFOX_OUT||path.join(process.cwd(),'firefox-parity-output');
const pages=[
...
try{
  for(const viewport of viewports){
    ...
    for(const item of pages){
      ...
      const response=await page.goto(ORIGIN+'/'+item.path+'?firefox-parity='+viewport.id,{
  ...
  const result={
    marker:failures.length?'FAIL_TARGETED_FIREFOX_23_ROUTE_PROOF':'PASS_TARGETED_FIREFOX_23_ROUTE_PROOF',
    browser:{engine:'firefox',version:browser.version(),automation:'playwright-1.63.0'},
    pages:pages.length,
