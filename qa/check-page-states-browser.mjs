...
const OUT = process.env.MODARYX_STATE_OUT || path.join(process.cwd(), 'page-state-proof-output');
const TEMP_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'modaryx-page-states-'));
...
async function runCheck(name, fn) {
  try {
    ...
    observations[name] = {status: 'PASS', detail};
  } catch (error) {
    failures.push(name + ': ' + error.message);
    observations[name] = {status: 'FAIL', detail: error.message};
  }
}
...
async function failRequests(cdp, urlPattern) {
...
try {
  ...
  await runCheck('search-empty-state', async () => {
    await navigate(cdp, 'search.html?state-proof=empty');
    await waitFor(cdp, `(() => {
      const input=document.querySelector('#site-search');
      return input && !input.disabled && document.querySelectorAll('#search-results .search-result').length > 0;
    })()`, 'search hydrated');

    const state = await evaluate(cdp, `(() => {
      const input=document.querySelector('#site-search');
      input.value='zzzz-modaryx-no-result-zzzz';
      input.dispatchEvent(new Event('input',{bubbles:true}));
      return {
        count:document.querySelector('#search-count')?.textContent||'',
        emptyHidden:document.querySelector('#search-empty')?.hidden,
        results:document.querySelectorAll('#search-results .search-result').length,
        state:document.querySelector('#search-state')?.textContent||''
      };
    })()`);
    assert(state.results === 0, 'search empty state still has results');
  ...
  await runCheck('search-error-recovery', async () => {
    ...
    const failed = await waitFor(cdp, `(() => {
      ...
      if (!retry || !/Index indisponible/.test(state.textContent||'')) return null;
      return {
        disabled:document.querySelector('#site-search')?.disabled,
        staticLinks:document.querySelectorAll('#search-results .search-result').length,
    ...
    await release();
    await evaluate(cdp, `document.querySelector('#search-state button')?.click(); true`);
    const recovered = await waitFor(cdp, `(() => {
      const input=document.querySelector('#site-search');
      const links=document.querySelectorAll('#search-results .search-result');
      return input && !input.disabled && links.length > 0 && document.activeElement===input
        ? {results:links.length, focused:document.activeElement.id}
        : null;
    })()`, 'search retry recovery');
    return {failed, recovered};
  });

  await runCheck('downloads-error-recovery', async () => {
    const release = await failRequests(cdp, '*downloads.json*');
    await navigate(cdp, 'downloads.html?state-proof=error');
    const failed = await waitFor(cdp, `(() => {
      const state=document.querySelector('[data-download-state]');
      const retry=document.querySelector('[data-download-retry]');
      if (!state || !retry || retry.hidden || !/Impossible de confirmer/.test(state.textContent||'')) return null;
      return {
        state:state.dataset.state||'',
        retryHidden:retry.hidden,
        artifacts:document.querySelectorAll('[data-download-artifacts] .download-artifact').length
      };
    ...
    await release();
    await evaluate(cdp, `document.querySelector('[data-download-retry]')?.click(); true`);
    const recovered = await waitFor(cdp, `(() => {
      const state=document.querySelector('[data-download-state]');
      const retry=document.querySelector('[data-download-retry]');
      if (!state || !retry) return null;
      const copy=state.textContent||'';
      return /Aucun téléchargement public déclaré disponible/.test(copy) && retry.hidden && document.activeElement===state
        ? {
            state:state.dataset.state||'',
            retryHidden:retry.hidden,
            focused:document.activeElement.hasAttribute('data-download-state'),
            artifacts:document.querySelectorAll('[data-download-artifacts] .download-artifact').length
          }
        : null;
  ...
  console.log(JSON.stringify({
    marker: failures.length ? 'FAIL_TARGETED_PAGE_STATES_BROWSER_PROOF' : 'PASS_TARGETED_PAGE_STATES_BROWSER_PROOF',
    checks: observations,
    failures
  }, null, 2));
  ...
  console.error(JSON.stringify({
    marker: 'FAIL_TARGETED_PAGE_STATES_BROWSER_PROOF',
    fatal: error.message,
    chromeStderr,
    failures
  }, null, 2));
