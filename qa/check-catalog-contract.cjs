...
async function load(payload, offline = false, failNetwork = false) {
  const calls = [];
  const filters = Array.from({length:6},()=>({disabled:false}));
  const context = { form:{querySelectorAll:()=>filters}, resetButton:{}, saveViewButton:{}, applyViewButton:{}, items: [], navigator: { onLine: !offline }, DATA_URL: './data/catalog.json',
    fetch: async () => { if (failNetwork) throw Error('network'); return { ok: true, json: async () => payload }; },
  ...
  return { context, calls, filters };
}
(async () => {
  const checks = [];
  let result = await load(data);
  assert.equal(result.context.items.length, 3); assert.ok(result.calls.includes('render'));
  assert.ok(result.filters.every(control=>!control.disabled));
  checks.push('Current records load with filters usable');
  ...
  for (const patch of bad) {
    ...
    assert.ok(result.filters.every(control=>control.disabled));
    assert.ok(result.context.resetButton.disabled && result.context.saveViewButton.disabled && result.context.applyViewButton.disabled);
    assert.match(result.context.stateNode.textContent,/Filtres indisponibles/);
  }
  ...
  for (const offline of [false,true]) {
    result = await load(data,offline,true);
    assert.deepEqual(result.calls,['static']);
    assert.ok(result.filters.every(control=>control.disabled));
    assert.ok(result.context.resetButton.disabled && result.context.saveViewButton.disabled && result.context.applyViewButton.disabled);
    assert.match(result.context.stateNode.textContent,/fiches et favoris|Fiches et favoris/);
    if(offline) assert.match(result.context.stateNode.textContent,/Hors ligne/);
  }
  checks.push('Network rejection disables ineffective controls, preserves static actions and explains online/offline recovery');
  const report = { result: 'PASS', scope: 'Seven grouped Node VM source scenarios, including ten malformed variants; not native browser proof', checks };
