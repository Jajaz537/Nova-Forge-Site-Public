...
const TEMP_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'modaryx-studio-schema-state-'));
...
async function failSchemaRequests(cdp) {
  await cdp.send('Network.enable');
  await cdp.send('Network.setBypassServiceWorker', {bypass: true});
  await cdp.send('Fetch.enable', {
    patterns: [{urlPattern: '*schemas/universal-mod-manifest.schema.json*', requestStage: 'Request'}]
  });
...
const validDraftScript = `(() => {
  const values={
    'content-id':'vf-schema-recovery',
    'content-version':'1.0.0',
    'content-name':'VF Schema Recovery',
    'game-id':'demo-game',
    'game-name':'Demo Game',
...
try {
  ...
  await runCheck('schema-unavailable-fail-closed', async () => {
    ...
    const unavailable = await waitFor(cdp, `(() => {
      const schema=document.querySelector('#schema-status')?.textContent||'';
      return /Schéma public indisponible/.test(schema)
        ? {schema,status:document.querySelector('#studio-status')?.textContent||''}
        : null;
    })()`, 'schema unavailable state');

    await evaluate(cdp, validDraftScript);
    await evaluate(cdp, `document.querySelector('#save-draft')?.click(); true`);
    const blocked = await waitFor(cdp, `(() => {
      const status=document.querySelector('#studio-status')?.textContent||'';
      return /Brouillon bloqué/.test(status)
        ? {
            status,
            schema:document.querySelector('#schema-status')?.textContent||'',
            stored:localStorage.getItem(${JSON.stringify(STORAGE_KEY)})
          }
        : null;
    })()`, 'schema fail-closed save');

    assert(blocked.stored === null, 'draft was persisted while schema unavailable');
    await release();
    return {unavailable, blocked};
  });

  await runCheck('schema-recovery-after-reload', async () => {
    await navigate(cdp, 'creator-studio.html?state-proof=schema-recovered');
    await evaluate(cdp, validDraftScript);
    await evaluate(cdp, `document.querySelector('#save-draft')?.click(); true`);
    ...
    const saved = await waitFor(cdp, `(() => {
      ...
      const status=document.querySelector('#studio-status')?.textContent||'';
      if (!raw || !/sauvegardé uniquement dans ce navigateur/i.test(status)) return null;
      try {
        const parsed=JSON.parse(raw);
        return {
          status,
          schema:document.querySelector('#schema-status')?.textContent||'',
          distribution:parsed?.manifest?.distribution,
          releaseReceipt:parsed?.manifest?.releaseReceipt,
          contentId:parsed?.manifest?.content?.id
        };
      } catch {
        return null;
      }
    })()`, 'schema recovery save');

    assert(saved.contentId === 'vf-schema-recovery', 'recovered draft content id mismatch');
    assert(saved.distribution?.state === 'locked' && saved.distribution?.downloadable === false, 'recovered draft distribution guard drifted');
    assert(saved.releaseReceipt === null, 'recovered draft releaseReceipt must remain null');
    assert(!/indisponible/i.test(saved.schema), 'schema still reported unavailable after recovery');
    await evaluate(cdp, `localStorage.removeItem(${JSON.stringify(STORAGE_KEY)}); true`);
    return saved;
  });
  ...
  console.log(JSON.stringify({
    marker: failures.length ? 'FAIL_TARGETED_STUDIO_SCHEMA_STATE' : 'PASS_TARGETED_STUDIO_SCHEMA_STATE',
    checks,
    failures
  }, null, 2));
  ...
  console.error(JSON.stringify({
    marker: 'FAIL_TARGETED_STUDIO_SCHEMA_STATE',
    fatal: error.message,
    chromeStderr,
    failures
  }, null, 2));
