(() => {
  ...
  const prefetchAllowed = !connection?.saveData && !['slow-2g', '2g'].includes(connection?.effectiveType || '');
  const prefetched = new Set();
  let intentTimer = 0;

  const eligibleLink = (node) => {
  ...
  const prefetch = (url) => {
    if (!prefetchAllowed || !url || prefetched.size >= 6 || prefetched.has(url.href)) return;
  ...
  const schedulePrefetch = (event) => {
    const url = eligibleLink(event.target);
    if (!url) return;
    clearTimeout(intentTimer);
    intentTimer = window.setTimeout(() => prefetch(url), 140);
  };
  const cancelPrefetch = () => clearTimeout(intentTimer);
